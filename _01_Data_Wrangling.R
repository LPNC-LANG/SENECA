...
##########################################################################################
  ...
  Consensus_vector <- read_excel("Consensus_vectors_CamCAN.xlsx", sheet = "FINAL") %>%
    mutate_at(vars(starts_with("Consensus")), funs(as.character(as.numeric(.)))) %>% 
    # Switching RS NET2 for RS NET3, error from Graphvar software
    # RS NET1 = Associative
    # RS NET2 = Sensorimotor
    # RS NET3 = Low-level attentional
    # RS NET4 = Control-executive
    mutate(Consensus_vector_0.15 = ifelse(Consensus_vector_0.15 == 2, 3, 
                                          ifelse(Consensus_vector_0.15 == 3, 2, Consensus_vector_0.15)))
  
  meta_data_0 <- merge(Network_overlap_InLang[,1:8], Consensus_vector, by = "Region")
