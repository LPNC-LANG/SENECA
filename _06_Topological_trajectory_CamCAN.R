...
cog_measures_ComStruct <- Cog_data_ILR_ComStruct[, c(3:8, 10:11)] %>%
  scale(.) %>% 
  as.data.frame() 
...
trajectory <- function(list_community, list_RSN, threshold, AGE_MIDDLE, AGE_OLD) {
  ...
  ##############################################################################
    modular <<- tmp_cluster_final %>%
      ...
      group_by(Consensus_vector_0.15, `1st_network`, Region, Age_group, Subj_ID, MODULAR) %>%
      summarise(n = n()) %>%
      mutate(freq = n / sum(n)) %>%
      dplyr::select(-n) %>%
      spread(MODULAR, freq) %>%
      mutate_all(., ~ replace(., is.na(.), 0)) %>%
      group_by(Consensus_vector_0.15, `1st_network`, Region, Age_group) %>%
      summarize_at(vars(Connector, Provincial, Satellite, Peripheral), mean) %>%
      ungroup() %>%
      arrange(Region) %>%
      pivot_longer(
        cols = !c("Consensus_vector_0.15", "1st_network", "Region", "Age_group"),
...
write.csv(flexibility_score, "rs_lang.csv")
