...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      p("Visit", em(a("github.com/lpnc-lang/SENECA", href = "https://github.com/lpnc-lang/SENECA")), "for detailed on the data analysis pipeline"),
      checkboxGroupInput("subsystem_choice",
                         h4("Pick at least one rs-LANG subsystem"),
                         choices = list(
                           "Associative" = "1",
                           "Sensorimotor" = "2",
                           "Low-level attentional" = "3",
                           "Control-executive" = "4"
                         )
      ),
    ...
    if (length(plot_button_subsystem()) > 1) {
      ...
      for (i in 1:length(plot_button_subsystem())) {
        ...
        subsystem_label_df <- subsystem_label_df %>% as.data.frame() %>% 
          mutate(V1 = ifelse(V1 == 1, "Associative", ifelse(V1 == 3, "Low-level attentional", 
