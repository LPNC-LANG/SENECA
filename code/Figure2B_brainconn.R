...
young_subj <- read_excel("./data/brainconn/brainconn_viz_R.xlsx",
  ...
  rownames_to_column() %>%
  slice(row_number(c(
    5, 7, 11, 15, 16, 18, 19, 22, 23, 25, 36, 42,
...
p_young <- brainconn_custom(
  atlas = "LANG_Atlas_young", conmat = young_subj, view = "top",
...
p_middle <- brainconn_custom(
  atlas = "LANG_Atlas_middle", conmat = middle_subj, view = "top",
  all.nodes = TRUE, edge.alpha = 0.8, edge.color = "black", edge.width = .3,
  node.size = 8, title = "", show.legend = F
)
...
p_old <- brainconn_custom(
  atlas = "LANG_Atlas_old", conmat = old_subj, view = "top",
  all.nodes = TRUE, edge.alpha = 0.8, edge.color = "black", edge.width = .3,
  node.size = 8, title = "", show.legend = F
)
...
brainconn_custom <- function(atlas, background = "ICBM152", view = "top", conmat = NULL,
  ...
  if (view == "ortho") {
    ...
    p <- plot_grid(ortho_list[[1]], right_col, rel_widths = c(
      1.8,
      1.2
    ))
    return(p)
  ...
  p <- build_plot(
    conmat = conmat, data = data, background = background,
...
build_plot <- function(conmat, data, data.row = NULL, data.col = NULL, background,
  ...
  if (directed == F && weighted == F) {
    p <- ggraph(layout, circular = FALSE) + annotation_custom(background,
      ...
      geom_edge_link(
        color = edge.color, edge_width = edge.width,
  ...
  if (view == "left") {
    p <- p + coord_fixed(xlim = c(-64, 98), ylim = c(
      -44,
      76
    ))
  }
  if (view == "right") {
    p <- p + coord_fixed(xlim = c(-98, 64), ylim = c(
      -44,
      76
    ))
  }
  if (directed == F) {
    ifelse(node.color == "network", p <- p + geom_node_point(
      size = node.size,
  ...
  p <- p + theme_bw(base_size = 25, base_family = "serif") + theme(
