...
Dimension1 <- c(
  cc_loadings$corr.Y.yscores[, 1][1],
...
Dimension2 <- c(
  cc_loadings$corr.Y.yscores[, 2][1],
...
correlational_space <- cbind(Dim1 = Dimension1$Dim1, Dim2 = Dimension2$Dim2, label) %>%
  as.data.frame() %>%
...
# RS NET1
ggplot(
  correlational_space %>% slice(1:8, 9:12),
...
# RS NET2
ggplot(
  correlational_space %>% slice(1:8, 13:16),
...
# RS NET3
ggplot(
  correlational_space %>% slice(1:8, 17:20),
  aes(as.numeric(Dim2), as.numeric(Dim1), label = label)
) +
  geom_hline(yintercept = 0, color = "grey", linetype = "dashed") +
  geom_vline(xintercept = 0, color = "grey", linetype = "dashed") +
...
# RS NET4
ggplot(
  correlational_space %>% slice(1:8, 21:24),
  aes(as.numeric(Dim2), as.numeric(Dim1), label = label)
) +
  geom_hline(yintercept = 0, color = "grey", linetype = "dashed") +
  geom_vline(xintercept = 0, color = "grey", linetype = "dashed") +
